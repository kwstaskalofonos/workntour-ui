...
export function get<T>(uri: string): Promise<T | any> {
  return new Promise((resolve, reject) =>
    fetch(Constants.getApiUrl() + uri, headers("GET"))
      .then(parseResponse)
...
export async function put<T>(uri: string, data: any): Promise<T | any> {
  return new Promise((resolve, reject) =>
    fetch(Constants.getApiUrl() + uri, headers("PUT", data))
      .then(parseResponse)
      .then((response: GenericResponse) => {
        if (response.ok) {
          return resolve(response.data);
...
export async function postMultipart<T>(
  ...
  return new Promise((resolve, reject) =>
    fetch(Constants.getApiUrl() + uri, multipartHeaders(data, method))
      .then(parseResponse)
      .then((response: GenericResponse) => {
        if (response.ok) {
          return resolve(response.data);
...
export function login<T>(email: string, password: string): Promise<T | any> {
  return new Promise((resolve, reject) =>
    fetch(
      Constants.getApiUrl() + "login",
      headers("POST", null, email, password)
    )
      .then(parseResponse)
      .then((response: GenericResponse) => {
        if (response.ok) {
...
export function subscribe<T>(
  ...
  return new Promise((resolve, reject) =>
    fetch(
      Constants.getApiUrl() + "earlySubscription",
      headers("POST", null, email)
    )
      .then(parseResponse)
      .then((response: GenericResponse) => {
        if (response.ok) {
