...
export const validateUserEmail = (token: string | null): Promise<string> => {
  return new Promise<string>((resolve, reject) =>
    get(`emailVerificationTokenStatus/${token}`)
      .then((response) => {
        resolve(response);
      })
      .catch((error) => {
...
export const changeUserPassword = (email: string | null): Promise<string> => {
  return new Promise<string>((resolve, reject) =>
    get(`/password/change/send/${email}`)
      .then((response) => {
        resolve(response);
      })
      .catch((error) => {
...
export const registerAsTraveler = (
  form: Traveler,
  setIsLoading: any
): Promise<Traveler> => {
...
export const registerAsCompany = (
  form: Company,
  setIsLoading: any
): Promise<Company> => {
...
export const retrieveIndividualProfile =
  (): Promise<IndividualHostProfileDto> => {
...
export const doSetRole: ActionCreator<ThunkResult> =
  (role: string) => (dispatch) => {
    return new Promise<void>(() => {
      dispatch(authenticationSlice.actions.setRole({ role: role }));
    });
  };

export const doSetProfile: ActionCreator<ThunkResult> =
  (
    profile:
      | TravelerProfileDTO
      | IndividualHostProfileDto
...
export const updateUserInfo: ActionCreator<ThunkResult> =
  (role: string) => (dispatch) => {
    return new Promise<void>(() => {
      switch (role) {
        case Role.TRAVELER.valueOf(): {
          retrieveTravelerProfile().then((response) => {
            dispatch(
              authenticationSlice.actions.setProfile({ profile: response })
            );
          });
          break;
        }
        case Role.COMPANY_HOST.valueOf(): {
          retrieveCompanyProfile().then((response) => {
            dispatch(
              authenticationSlice.actions.setProfile({ profile: response })
            );
          });
        ...
        case Role.INDIVIDUAL_HOST.valueOf(): {
          retrieveIndividualProfile().then((response) => {
            dispatch(
              authenticationSlice.actions.setProfile({ profile: response })
            );
          });
...
export const updateTravelerProfile: ActionCreator<ThunkResult> =
  (data: FormData, setIsLoading: any, setFile: any) => (dispatch) => {
    return new Promise<TravelerProfileDTO>((resolve, reject) => {
...
export const updateCompanyProfile: ActionCreator<ThunkResult> =
  (data: FormData, setIsLoading: any, setFile: any) => (dispatch) => {
    return new Promise<CompanyHostProfileDto>((resolve, reject) => {
...
export const updateIndividualProfile: ActionCreator<ThunkResult> =
  (data: FormData, setIsLoading: any, setFile: any) => (dispatch) => {
    return new Promise<IndividualHostProfileDto>((resolve, reject) => {
